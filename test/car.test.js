...
describe('<Card />', () => {
  ...
  it(testCase.caseRender.title, async () => {
    ...
    let wrapper = mount(<Provider store={store}> <Card url={testCase.caseRender.data.update.url}/></ Provider> )
    expect(wrapper.find({'test-id':"star-war-card-link"}).exists())
    wrapper.find({'test-id':"star-war-card-link"}).simulate('click');
  
    expect(store.getActions()[0].url).toEqual(testCase.caseRender.data.update.url);
  });
